...
function displayedDatabaseUrl(url) {
    ...
    switch (command) {
        ...
        case 'exec': {
            ...
            if (result.rows.length > 0)
                output(result.rows, jsonMode);
            else
                output({ changes: result.changes, lastInsertRowid: result.lastInsertRowid }, jsonMode);
            conn.close();
            break;
        }
        case 'insert': {
        ...
        case 'shell': {
            ...
            rl.on('line', async (line) => {
                ...
                if (trimmed === 'help')
                    console.log('Commands: <sql>, help, exit');
                else if (trimmed) {
                    try {
