...
export const secretReferenceSchema = z
  ...
  .min(3)
  .max(160)
  .regex(/^[a-z0-9]+(?:[._-][a-z0-9]+)*$/, "Reference must use lower-case letters, digits, dots, hyphens, or underscores.");
...
export const jobSubmissionSchema = z.object({
  ...
  secretReferences: z.array(secretReferenceSchema).min(1).max(10),
  requiredScopes: z.array(scopeSchema).min(1).max(10),
...
export const ZERO_PLAINTEXT_INVARIANT =
  "Raw secret values must never be serialized to a browser, client SDK response, audit event, log entry, URL, or persistent client storage." as const;
