...
Run `node worker/accx-worker.mjs` as a **single scheduled invocation**. It asks `POST /api/v1/internal/dispatch` to process at most one queued job. A scheduler may invoke it frequently, but concurrent invocations are safe: the control plane atomically changes `queued` jobs to `running` and binds them to exactly one `workerId`. Replays return a sanitized state and do not call provider adapters twice.
