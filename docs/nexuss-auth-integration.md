...
A browser handoff does not expose an email-verification flag in the returned Nexuss user shape, so a new subject is not auto-linked by email during browser OAuth. It returns a deliberate `nexuss_auth_account_link_required` result when an existing local email matches. The explicit portable-token login is the migration path: the caller presents a user-owned, Nexuss-validated bearer key, and ACCX may link its returned email to the matching local account while storing only issuer and subject. An absent email never links by email. If the match is ambiguous, the flow fails safely instead of creating an account takeover path.
