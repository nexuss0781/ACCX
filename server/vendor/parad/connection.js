...
export function generateUrl(name, passphrase = '', gatewayUrl = '', project = null, token = '', email = '', password = '') {
    let userinfo = '';
    if (email && password) {
    ...
    const qs = [];
    if (passphrase)
        qs.push(`passphrase=${encodeURIComponent(passphrase)}`);
    if (gatewayUrl)
        qs.push(`gateway=${encodeURIComponent(gatewayUrl).replace(/%3A/g, ':').replace(/%2F/g, '/')}`);
    if (token && email && password)
        qs.push(`token=${encodeURIComponent(token)}`);
    if (qs.length)
        url += `?${qs.join('&')}`;
    return url;
}
export function dbStateKey(name, project = null) {
    if (project)
        return `${project}/${name}`;
    return name;
}
 ...
 */
export function getCanonicalDatabaseUrl(name) {
    ...
    if (!inferredName)
        throw new Error('No database name is configured; run parad init <name> first');
...
export async function connect(opts) {
    ...
    if (!resolvedPassphrase) {
        ...
        if (!fs.existsSync(resolvedPath)) {
            ...
            throw new Error(`No passphrase configured for existing database '${resolvedPath}'. ` +
