...
export default async function handler(req: ApiRequest, res: ApiResponse): Promise<void> {
  ...
  try { const input = schema.parse(req.body); sendJson(res, 200, await withControlPlaneDb(db => { const user = requireStepUp(db, req); return { imported: importEncryptedWorkspace(db, { workspaceId: input.workspaceId, actorId: user.id, bundle: input.bundle as never }) }; }, { write: true })); }
